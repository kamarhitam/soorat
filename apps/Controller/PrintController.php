...
class PrintController extends NG\Controller {
    ...
    public function init() {
        ...
        $this->render = $this->view->render = new Render();
                ...
                case "post":
                    if ($param2){
                        ...
                        if ($dataLetter) {
                            ...
                            $title = $helper->getArrayValue($dataPost, 'title');
                            $slug = $helper->getSlug($title);
                            $genData = $render->generateData(array("data" => $dataLetter), $codeLetter);
                            if ($param3) {
                                if ($param3 == "unsign"){
                                    $newGenData = array();
                                    if (is_array($genData)) {
                            ...
                            $dataContent = $render->render($content, $genData);
                            $dataContent = $render->clearFormat($dataContent);
                            // print_r($genData);
                ...
                case "laporan":
                    ...
                    if ($param2){
                        ...
                        if ($dataPost) {
                            ...
                            $dataPost["judul-surat"] = "$tagTitle " . $dataPost["title"];
                            ...
                            $dataLetterRaw = $clsLetter->fetch($type, $postId, $filterYear, $filterMonth, 0);
                            ...
                            if ($dataLetterRaw) {
                                ...
                                if ($dataLetter) {
                                                                            ...
                                                                            if ($selectedIdKey) {
                                                                                ...
                                                                                if ($arrSelectedIdKey) {
                                                                                    ...
                                                                                    if (count($arrSelectedIdKey) > 1) {
                                                                                        ...
                                                                                        } else if ($selectedIdKeyType == "post") {
                                                                                            $clsPost = new CmsPost();
                                                                                            $clsType = new CmsType();
                                                                                            $post = $clsPost->getBySlug($selectedIdKeyMeta);
                                                                                            if ($post) {
                                                                                                $typePost = $helper->getArrayValue($post, 'type');
                                                                                                $idPost = $helper->getArrayValue($post, 'id');
                                                                                                if ($typePost) {
                                                                                                    $cmsType = $clsType->getById($typePost);
                                                                                                    if ($cmsType) {
                                                                                                        $clsLetter = new PostData();
                                                                                                        if ($clsLetter) {
                                                                                                            $dataLetterRaw = $clsLetter->fetch($typePost, $idPost, 0, 0);
                                                                                                            if ($dataLetterRaw) {
                                                                                                                $dataLetter = $helper->getArrayValue($dataLetterRaw, 'data');
                                                                                                                if ($dataLetter) {
                                                                                                                    $selectList = array();
                                                                                                                    foreach ($dataLetter as $itemLetter) {
                                                                                                                        $detailKeys = $helper->getArrayValue($itemLetter, 'detail-keys');
                                                                                                                        $detailValues = $helper->getArrayValue($itemLetter, 'detail-values');
                                                                                                                        if ($detailKeys) {
                                                                                                                            $arrDetailValues = explode("***", $detailValues);
                                                                                                                    ...
                                                                                                                    if ($selectList) {
                                                                                                                        $itemValue = implode(', ', $selectList);
                                                                                                                        $item[$itemDetailKeys] = $itemValue;
                                                                                                                    }
                                                                                                                }
                                                                                                            }
                                                                                                        }
                                    ...
                                    $genData = $render->generateData($data, '');
                                    $dataHeader = $render->putTemplateReport("surat");
                                    $dataHeader = $render->render($dataHeader, $genData);
                                    $dataBody = $render->putTemplateReport("surat-body");
                                    $dataBody = $render->render($dataBody, $genData);
                                    $bodyContent = "";
                                    foreach ($rows as $row) {
                                        $bodyContent .= "\r" . $render->childRender($dataBody, array("data" => $row));
                                    }
