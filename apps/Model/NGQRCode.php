...
class NGQRCode {
    ...
    function makeImpl($test, $maskPattern) {
        ...
        for ($i = 0; $i < $this->moduleCount; $i++) {
            $this->modules[] = NGQRCode::createNullArray($this->moduleCount);
        }
        ...
        $data = NGQRCode::createData($this->typeNumber, $this->errorCorrectLevel, $dataArray);
    ...
    function createData($typeNumber, $errorCorrectLevel, $dataArray) {
        ...
        return NGQRCode::createBytes($buffer, $rsBlocks);
    }
    ...
    function createBytes(&$buffer, &$rsBlocks) {
        ...
        $dcdata = NGQRCode::createNullArray(count($rsBlocks) );
        $ecdata = NGQRCode::createNullArray(count($rsBlocks) );
        ...
        for ($r = 0; $r < $rsBlockCount; $r++) {
            ...
            $dcdata[$r] = NGQRCode::createNullArray($dcCount);
            ...
            $ecdata[$r] = NGQRCode::createNullArray($rsPoly->getLength() - 1);
        ...
        $data = NGQRCode::createNullArray($totalCodeCount);
